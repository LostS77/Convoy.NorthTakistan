NEWTAB("I. Mission:")
<font color='#BBBBBB' size='16' face='PuristaBold'>Task:</font><br/>
<br/>
(name of platoon) is to escort mission critical trucks from <marker name='StartPoint'>Camp Black Rose</marker> to <marker name='EndPoint'>Air Base Skywalker</marker>.<br/>
<br />
There has been a known IED threat in the area. Upon discovery, disarm and recover if possible or destroy.<br />
<br />
Motorized hostile forces have been sighted patrolling the desert.<br />
<br />
There is civilian presense in cities.  Try to keep any colaterial damage to a minimum.
<br />
<font color='#BBBBBB' size='16' face='PuristaBold'>Scouts report:</font><br />
<marker name='Route1'>Route 1</marker> has a high enemy presence and some IEDs.<br />
<marker name='Route2'>Route 2</marker>, high amount of IEDs and mines reported along with a unknown hostile composition in the vicinity.<br />
<marker name='Route3'>Route 3</marker> and <marker name='Route4'>Route 4</marker> are advised against taking due to heavy (what the threat is, probably mines/ieds)<br />
It is advised that the platoon stays on one of the routes as heavy anti-tank and anti-personnel mining operations were known to have taken place in the area.<br />
<br />
The platoon is to reach <marker name='EndPoint'>Air Base Skywalker</marker> by t+90 and prepare for follow on operations there.<br />
<br />
//This is how you create a <marker name='markerName'>marker</marker>.
ENDTAB;


NEWTAB("A. Enemy Forces:")
Air is crawling with Insurgents.
ENDTAB;

NEWTAB("B. Friendly Forces:")
US Army Platoon<br />
<br />
HQ<br />
Black Horse 1<br />
Black Horse 2<br />
Black Horse 3<br />
Heavy Rain (Weapons Squad)<br />
Medical Attachment<br />
Engineering Attachment<br/>
<br />
ENDTAB;

NEWTAB("II. Execution:")
Simply get the 3 supply trucks and your soldiers to <marker name='EndPoint'>Air Base Skywalker</marker><br />
CO picks the route<br />
<br />
ENDTAB;

NEWTAB("III. Service Support:")
1x Land Rover UGV<br />
2x M1237 M2 MRAP's<br />
2x M1237 Mk19 MRAP's<br />
1x M1277 Crows M2 (HQ)<br />
1x M1230A1 MEDEVAC (Stitching can only be done from within Medical vehicle)<br />
1x M977A4-B M2 Repair Truck (Needs to make it to <marker name='EndPoint'>Air Base Skywalker</marker>)<br />
1x M977A4-B M2 Ammo Truck (Needs to make it to <marker name='EndPoint'>Air Base Skywalker</marker>)<br />
1x M978A4-B Fuel Truck (Needs to make it to <marker name='EndPoint'>Air Base Skywalker</marker>)<br />
ENDTAB;

NEWTAB("IV. Command & Signal:")
<font color='#BBBBBB' size='16' face='PuristaBold'>Long Range:</font><br/>
Platoon Net - Channel 1<br />
<br />
<font color='#BBBBBB' size='16' face='PuristaBold'>Short Range:</font><br/>
Black Horse 1 - Channel 1<br />
Black Horse 2 - Channel 2<br />
Black Horse 3 - Channel 3<br />
Heavy Rain - Channel 4<br />
HQ - Channel 5<br />
Medical Attachment - Channel 6<br />
Enginneering Attachment - Channel 7<br />
<br />
<br />
*** You will need to set your own channels.  I am too lazy to set them for you ***<br />
ENDTAB;